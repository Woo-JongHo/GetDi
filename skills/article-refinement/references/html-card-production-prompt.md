...
- **배경은 전부 검정 계열이다.** 카드마다 배경색이 달라지면 피드에서
  한 시리즈로 읽히지 않는다. 유형 구분은 배경이 아니라 강조색이 한다.
- 글자는 off-white, 강조색은 검정 위에서 읽히도록 밝고 채도가 있어야 한다.
- 표지는 원문 이미지가 있으면 어두운 오버레이 위에 제목을 세운다.
